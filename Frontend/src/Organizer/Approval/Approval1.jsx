...
const AdminApproval = () => {
  ...
  const confirmStatusUpdate = async () => {
    try {
      await updateBookingStatus(confirmModal.id, confirmModal.status);

      setBookings((prev) =>
        prev.map((b) =>
          b.id === confirmModal.id
            ? { ...b, status: confirmModal.status }
            : b
        )
      );

      if (selectedBooking && selectedBooking.id === confirmModal.id) {
        setSelectedBooking({
          ...selectedBooking,
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/40">
          <div className="bg-white p-6 rounded-lg shadow-lg w-80">
            <h2 className="text-lg font-bold mb-3">Confirm</h2>

            <p className="mb-4">
              Are you sure you want to{" "}
              <b>{confirmModal.status}</b> this booking?
            </p>
